...
export default function Download({ getDownloads, requestInit }: Props) {
  ...
  // SEB has a z-index 1000 footer that otherwise would cover this
  return (
    <div className="fixed right-4 bottom-4 z-[9999] h-32 w-64 rounded-lg bg-white p-4 font-sans shadow-lg ring-1 ring-black/5">
      {state.error && (
        <>
          <div>{state.error}</div>
          {goBack}
